...
fn main() -> Result<(), io::Error> {
    ...
    let args = Args::parse();
    let opcode_str = args.opcode_str.clone().clone();
    ...
    let (mut constraint_info, general_lookup_info) =
        extract_constraints_and_range::<KoalaBear, LtChip<KoalaBear>>(&air, NUM_LT_COLS, prime, args.method == "bb" && !args.no_simplify);
