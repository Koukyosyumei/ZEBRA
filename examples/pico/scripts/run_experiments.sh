...
# Experiment 2 – Range sweep   : vary --range-interval = {0,2,4,8,16,32,64,128}
...
CHIP_OPCODES[sll]="SLL"   # --opcode-str is ignored by sll binary; "SLL" is just a label
...
# Write a temporary JSON config with the given num_workers and echo its path.
make_config() {
    local workers=$1
    local tmpfile
...
# Run one verification trial and save the YAML report.
#   run_one <chip> <opcode> <config_file> <range_interval> <output_yaml>
run_one() {
