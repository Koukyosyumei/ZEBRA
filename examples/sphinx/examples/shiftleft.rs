...
use sphinx_core::alu::{ShiftLeft, ShiftLeftCols, NUM_SHIFT_LEFT_COLS};
...
fn main() -> Result<(), io::Error> {
    ...
    let (mut constraint_info, general_lookup_info) =
        extract_constraints_and_range::<BabyBear, ShiftLeft>(
            &air,
            NUM_SHIFT_LEFT_COLS,
            prime,
    ...
    if args.sparsity {
        print_all_sparsity_reports(&[("ShiftLeft", &constraint_info)]);
        return Ok(());
    }
