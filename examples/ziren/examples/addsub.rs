...
fn main() -> Result<(), io::Error> {
    ...
    let (mut constraint_info, _general_lookup_info) =
        extract_constraints_and_range::<KoalaBear, AddSubChip>(&air, NUM_ADD_SUB_COLS, prime, args.method == "bb" && !args.no_simplify);
    constraint_info
        .refinable_cols
    ...
    if args.sparsity {
        print_all_sparsity_reports(&[("AddSub", &constraint_info)]);
        return Ok(());
    }
