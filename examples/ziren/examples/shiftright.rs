...
use zkm_core_machine::alu::{ShiftRightCols, NUM_SHIFT_RIGHT_COLS};
use zkm_core_machine::ShiftRightChip;
...
const fn make_col_map() -> ShiftRightCols<usize> {
    let indices_arr = indices_arr::<{ NUM_SHIFT_RIGHT_COLS }>();
    unsafe { transmute::<[usize; NUM_SHIFT_RIGHT_COLS], ShiftRightCols<usize>>(indices_arr) }
}
...
fn main() -> Result<(), io::Error> {
    ...
    let (mut constraint_info, general_lookup_info) = extract_constraints_and_range::<
        KoalaBear,
        ShiftRightChip,
    >(&air, NUM_SHIFT_RIGHT_COLS, prime, args.method == "bb" && !args.no_simplify);
    ...
    if args.sparsity {
        print_all_sparsity_reports(&[("ShiftRight", &constraint_info)]);
        return Ok(());
    }
