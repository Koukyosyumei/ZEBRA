...
use zebra_openvm::utils::{extract_lt_constraints, make_lt_row, to_limbs, BABY_BEAR_PRIME};

pub fn get_opcode(opcode_str: &str) -> LessThanOpcode {
    match opcode_str {
        "SLT"  => LessThanOpcode::SLT,
        "SLTU" => LessThanOpcode::SLTU,
        _ => panic!("unsupported opcode: {opcode_str}; use SLT or SLTU"),
    }
}
...
fn main() -> Result<(), io::Error> {
    ...
    let (mut constraint_info, general_lookup_info) =
        extract_lt_constraints(BABY_BEAR_PRIME);
    ...
    if args.sparsity {
        print_all_sparsity_reports(&[("Lt", &constraint_info)]);
        return Ok(());
    }
    ...
    for _ in 0..args.num_trial {
        ...
        let base_abs_main_trace_data = vec![make_lt_row(b_limbs, c_limbs, op)];
