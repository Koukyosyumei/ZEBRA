...
use zebra_openvm::utils::{extract_mul_constraints, make_mul_row, to_limbs, BABY_BEAR_PRIME};
...
fn main() -> Result<(), io::Error> {
    ...
    if args.sparsity {
        print_all_sparsity_reports(&[("Mul", &constraint_info)]);
        return Ok(());
    }
    ...
    for _ in 0..args.num_trial {
        ...
        let base_abs_main_trace_data = vec![make_mul_row(b_limbs, c_limbs)];

        let program_info = ProgramInfo {
            program_str: format!("MUL({x:#010x}, {y:#010x})"),
            program_len: 1,
        };
        ...
        println!("MUL({x:#010x}, {y:#010x}): {:?}", result);
        let r = result.unwrap();
