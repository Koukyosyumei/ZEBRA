...
use zebra_openvm::utils::{
    extract_shift_constraints, make_shift_row, to_limbs, BABY_BEAR_PRIME,
};
...
fn main() -> Result<(), io::Error> {
    ...
    if args.sparsity {
        print_all_sparsity_reports(&[("Shift", &constraint_info)]);
        return Ok(());
    }
    ...
    for _ in 0..args.num_trial {
        ...
        let base_abs_main_trace_data = vec![make_shift_row(b_limbs, c_limbs, ShiftOpcode::SRL)];

        let program_info = ProgramInfo {
            program_str: format!("SRL({x:#010x}, {y})"),
            program_len: 1,
        };
