...
fn get_target_program<Val: StarkField>() -> Vec<InstructionWord<i32>> {
    ...
    program.extend([
        ...
        InstructionWord {
            ...
            operands: Operands([-4, 3 * bytes_per_instr, 0, 0, 0]),
        },
        // --- callee ---
        // mem[-8] = 42
        InstructionWord {
            opcode: <Imm32Instruction as Instruction<BasicMachine<Val>, Val>>::OPCODE,
            operands: Operands([-8, 42, 0, 0, 0]),
        },
        // stop
        InstructionWord {
...
fn main() -> Result<(), io::Error> {
    ...
    let a = AbstractTrace::new(base_abs_main_trace_data.clone());
    println!("{}", a);

    /*
    let mut bb = base_abs_main_trace_data.clone();
    bb[3][1] = AbstractInterval::from_i64(3);
    bb[2][18] = AbstractInterval::zero();
    bb[4][58] = AbstractInterval::zero();
    bb[5][58] = AbstractInterval::zero();
    bb[4][0] = AbstractInterval::zero();
    bb[5][0] = AbstractInterval::zero();
    let mut bt = AbstractTrace::new(bb);
    let flag = eval_constraints(&bt, Some(&public_vals), &constraints, prime);
    println!("{}", constraints.air_constraints[52]);
    println!("{}", bt);
    println!("flag: {:?}", flag);
    */
