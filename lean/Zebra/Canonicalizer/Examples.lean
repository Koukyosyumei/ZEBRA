...
/-- Shared guarantee when a table-specific tuple is interpreted as an abstract
    record identity. This is the form used when the proof should not identify
    semantic records directly with table tuples. -/
theorem one_to_one_for_record_id_config {Record TableTuple Identity : Type}
    [DecidableEq TableTuple] [DecidableEq Identity]
    (cfg : Generic.Config TableTuple)
    (tableId : TableTuple → Identity)
    (recordId : Generic.RecordIdentity Record Identity)
    (generateTable : Generic.RecordSet Record → Trace)
    (hgen : Generic.TableGeneratorFaithfulToIds
      cfg tableId recordId generateTable)
    (records₁ records₂ : Generic.RecordSet Record) :
    Generic.recordIdsOfTable cfg tableId (generateTable records₁) =
      Generic.recordIdsOfTable cfg tableId (generateTable records₂) ↔
    records₁ = records₂ :=
  Generic.recordIds_generated_eq_iff_records_eq
    cfg tableId recordId generateTable hgen records₁ records₂
...
/-- Guarantee for lookup-driven ALU tables whose concrete operand columns are
    ...
    the example file. -/
theorem lookup_driven_alu_one_to_one {Record Identity : Type} [DecidableEq Identity]
    (cfg : Generic.Config ALU.Tuple)
    (tableId : ALU.Tuple → Identity)
    (recordId : Generic.RecordIdentity Record Identity)
    (generateTable : Generic.RecordSet Record → Trace)
    (hgen : Generic.TableGeneratorFaithfulToIds
      cfg tableId recordId generateTable)
    ...
    Generic.recordIdsOfTable cfg tableId (generateTable records₁) =
      Generic.recordIdsOfTable cfg tableId (generateTable records₂) ↔
    records₁ = records₂ :=
  one_to_one_for_record_id_config cfg tableId recordId generateTable hgen records₁ records₂
...
theorem cpu_one_to_one_for_config {Record Identity : Type}
    [DecidableEq Record] [DecidableEq Identity]
    (cfg : CPU.Config)
    (tableId : CPU.RecordRepr → Identity)
    (recordId : Generic.RecordIdentity Record Identity)
    (generateTable : Generic.RecordSet Record → Trace)
    (hgen : CPU.TableGeneratorFaithfulToIds cfg tableId recordId generateTable)
    (records₁ records₂ : Generic.RecordSet Record) :
    CPU.recordIdsOfTable cfg tableId (generateTable records₁) =
      CPU.recordIdsOfTable cfg tableId (generateTable records₂) ↔
    records₁ = records₂ :=
  CPU.recordIds_generated_eq_iff_records_eq
    cfg tableId recordId generateTable hgen records₁ records₂
...
theorem add_one_to_one {Record Identity : Type} [DecidableEq Identity] (isReal : Row → Bool)
    (tableId : ALU.Tuple → Identity)
    (recordId : Generic.RecordIdentity Record Identity)
    (generateTable : Generic.RecordSet Record → Trace)
    (hgen : Generic.TableGeneratorFaithfulToIds
      (addConfig isReal) tableId recordId generateTable)
    (records₁ records₂ : Generic.RecordSet Record) :
    Generic.recordIdsOfTable (addConfig isReal) tableId (generateTable records₁) =
      Generic.recordIdsOfTable (addConfig isReal) tableId (generateTable records₂) ↔
    records₁ = records₂ :=
  one_to_one_for_record_id_config
    (addConfig isReal) tableId recordId generateTable hgen records₁ records₂

/-- SP1 SUB canonicalizer one-to-one guarantee. -/
theorem sub_one_to_one {Record Identity : Type} [DecidableEq Identity] (isReal : Row → Bool)
    (tableId : ALU.Tuple → Identity)
    (recordId : Generic.RecordIdentity Record Identity)
    (generateTable : Generic.RecordSet Record → Trace)
    (hgen : Generic.TableGeneratorFaithfulToIds
      (subConfig isReal) tableId recordId generateTable)
    (records₁ records₂ : Generic.RecordSet Record) :
    Generic.recordIdsOfTable (subConfig isReal) tableId (generateTable records₁) =
      Generic.recordIdsOfTable (subConfig isReal) tableId (generateTable records₂) ↔
    records₁ = records₂ :=
  one_to_one_for_record_id_config
    (subConfig isReal) tableId recordId generateTable hgen records₁ records₂
...
theorem memoryInstrs_one_to_one {Record Identity : Type} [DecidableEq Identity]
    (isReal : Row → Bool)
    (tableId : Memory.MemoryOpTuple → Identity)
    (recordId : Generic.RecordIdentity Record Identity)
    (generateTable : Generic.RecordSet Record → Trace)
    (hgen : Generic.TableGeneratorFaithfulToIds
      (memoryInstrsConfig isReal) tableId recordId generateTable)
    (records₁ records₂ : Generic.RecordSet Record) :
    Generic.recordIdsOfTable (memoryInstrsConfig isReal) tableId (generateTable records₁) =
      Generic.recordIdsOfTable (memoryInstrsConfig isReal) tableId (generateTable records₂) ↔
    records₁ = records₂ :=
  one_to_one_for_record_id_config
    (memoryInstrsConfig isReal) tableId recordId generateTable hgen records₁ records₂

/-- SP1 CPU canonicalizer one-to-one guarantee. -/
theorem cpu_one_to_one {Record Identity : Type} [DecidableEq Record] [DecidableEq Identity]
    (isReal isOpAWrite : Row → Bool)
    (tableId : CPU.RecordRepr → Identity)
    (recordId : Generic.RecordIdentity Record Identity)
    (generateTable : Generic.RecordSet Record → Trace)
    (hgen : CPU.TableGeneratorFaithfulToIds
      (cpuConfig isReal isOpAWrite) tableId recordId generateTable)
    (records₁ records₂ : Generic.RecordSet Record) :
    CPU.recordIdsOfTable (cpuConfig isReal isOpAWrite) tableId (generateTable records₁) =
      CPU.recordIdsOfTable (cpuConfig isReal isOpAWrite) tableId (generateTable records₂) ↔
    records₁ = records₂ :=
  cpu_one_to_one_for_config
    (cpuConfig isReal isOpAWrite) tableId recordId generateTable hgen records₁ records₂
...
theorem memoryReadWrite_one_to_one {Record Identity : Type} [DecidableEq Identity]
    (isReal : Row → Bool)
    (tableId : Memory.MemoryOpTuple → Identity)
    (recordId : Generic.RecordIdentity Record Identity)
    (generateTable : Generic.RecordSet Record → Trace)
    (hgen : Generic.TableGeneratorFaithfulToIds
      (memoryReadWriteConfig isReal) tableId recordId generateTable)
    (records₁ records₂ : Generic.RecordSet Record) :
    Generic.recordIdsOfTable (memoryReadWriteConfig isReal) tableId (generateTable records₁) =
      Generic.recordIdsOfTable (memoryReadWriteConfig isReal) tableId (generateTable records₂) ↔
    records₁ = records₂ :=
  one_to_one_for_record_id_config
    (memoryReadWriteConfig isReal) tableId recordId generateTable hgen records₁ records₂
...
theorem cpu_one_to_one {Record Identity : Type} [DecidableEq Record] [DecidableEq Identity]
    ...
    (generateTable : Generic.RecordSet Record → Trace)
    (hgen : CPU.TableGeneratorFaithfulToIds
      (cpuConfig isReal) tableId recordId generateTable)
    (records₁ records₂ : Generic.RecordSet Record) :
    CPU.recordIdsOfTable (cpuConfig isReal) tableId (generateTable records₁) =
      CPU.recordIdsOfTable (cpuConfig isReal) tableId (generateTable records₂) ↔
    records₁ = records₂ :=
  cpu_one_to_one_for_config
    (cpuConfig isReal) tableId recordId generateTable hgen records₁ records₂
...
theorem div_one_to_one {Record Identity : Type} [DecidableEq Identity] (isReal : Row → Bool)
    (tableId : ALU.Tuple → Identity)
    (recordId : Generic.RecordIdentity Record Identity)
    (generateTable : Generic.RecordSet Record → Trace)
    (hgen : Generic.TableGeneratorFaithfulToIds
      (divConfig isReal) tableId recordId generateTable)
    (records₁ records₂ : Generic.RecordSet Record) :
    Generic.recordIdsOfTable (divConfig isReal) tableId (generateTable records₁) =
      Generic.recordIdsOfTable (divConfig isReal) tableId (generateTable records₂) ↔
    records₁ = records₂ :=
  one_to_one_for_record_id_config
    (divConfig isReal) tableId recordId generateTable hgen records₁ records₂

/-- Ziren REM canonicalizer one-to-one guarantee. -/
theorem rem_one_to_one {Record Identity : Type} [DecidableEq Identity] (isReal : Row → Bool)
    (tableId : ALU.Tuple → Identity)
    (recordId : Generic.RecordIdentity Record Identity)
    (generateTable : Generic.RecordSet Record → Trace)
    (hgen : Generic.TableGeneratorFaithfulToIds
      (remConfig isReal) tableId recordId generateTable)
    (records₁ records₂ : Generic.RecordSet Record) :
    Generic.recordIdsOfTable (remConfig isReal) tableId (generateTable records₁) =
      Generic.recordIdsOfTable (remConfig isReal) tableId (generateTable records₂) ↔
    records₁ = records₂ :=
  one_to_one_for_record_id_config
    (remConfig isReal) tableId recordId generateTable hgen records₁ records₂
...
theorem memory_one_to_one {Record Identity : Type} [DecidableEq Record] [DecidableEq Identity]
    (isRead isWrite : Row → Bool)
    (tableId : Memory.AccessTuple → Identity)
    (recordId : Generic.RecordIdentity Record Identity)
    (generateTable : Generic.RecordSet Record → Trace)
    (hgen : Memory.AccessTableGeneratorFaithfulToIds
      (memoryConfig isRead isWrite) tableId recordId generateTable)
    (records₁ records₂ : Generic.RecordSet Record) :
    Memory.recordIdsOfAccessTable
        (memoryConfig isRead isWrite) tableId (generateTable records₁) =
      Memory.recordIdsOfAccessTable
        (memoryConfig isRead isWrite) tableId (generateTable records₂) ↔
    records₁ = records₂ :=
  Memory.recordIdsOfAccessTable_eq_iff_records_eq
    (memoryConfig isRead isWrite) tableId recordId generateTable hgen records₁ records₂
